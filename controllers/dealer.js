...
const convertCsvToJson = async (csvFile, dealerId) => {
    try {
        ...
        for (let i = 1; i < rows.length; i++) {
            ...
            // console.log(rowData["Extra_Photos"]);
            ...
            if (checkExist.length) {
                ...
                if (dataRow.carfaxlink === null || dataRow.carfaxlink === '' || dataRow.carfaxlink === undefined) {
                    try {
                        ...
                        console.log("Failed to check Carfax report")
                    }
    ...
    try {
      const mailOptions = {
        ...
        <div style="font-family: Arial, sans-serif; padding: 20px; background: #f9f9f9; color: #333;">
          <div style="max-width: 500px; margin: auto; background: #ffffff; border-radius: 10px; padding: 30px; box-shadow: 0 0 10px rgba(0,0,0,0.1);">
            <h2 style="color: #007BFF;">UOBO Verification Code</h2>
            <p>Hello,</p>
            <p>Your OTP code is:</p>
            <div style="font-size: 24px; font-weight: bold; margin: 20px 0; color: #333; background: #f0f0f0; padding: 10px 20px; display: inline-block; border-radius: 6px;">
              ${otp}
            </div>
            <p>This code is valid for 5 minutes. Please do not share it with anyone.</p>
            <br/>
            <p style="font-size: 12px; color: #888;">If you didn't request this, please ignore this email.</p>
            <p style="font-size: 14px; margin-top: 30px;">– UOBO Team</p>
          </div>
        </div>
      `
      };
        ...
        try {
            ...
            if (account != null) {
                ...
                const accountLink = await stripe.accountLinks.create({
                    account: account.id,
                    refresh_url: 'https://uobo-api.onrender.com/retry-stripe-onboarding',
                    return_url: 'https://uobo-api.onrender.com/dealer-dashboard',
    ...
    updateDealerInventory: async (req, res, next) => {
        try {
            ...
            if (csvFile) {
                ...
                return res.status(200).json({ IsSuccess: true, Count: inventory_data.length, Data: inventory_data, Message: 'Inventory updated successfully' });
            } else {
